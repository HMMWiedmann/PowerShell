...
<#  Aufm DNS Server
    Host-A für Webserver (SubCA) erstellen pki.mopw-center.de
#>
...
Install-AdcsCertificationAuthority -CAType StandaloneRootCA `
                                   ...
                                   -CACommonName 'MOPW-CENTER-RootCA' `
...
<# CDP (Sperrliste) konfigurieren
    ...
    - ldap:///CN=<CATruncatedName><CRLNameSuffix>,CN=<ServerShortName>,CN=CDP,CN=Public Key Services,CN=Services,CN=Configuration,DC=MOPW-CENTER,DC=DE
    ...
    - http://pki.mopw-center.de/CertEnroll/<CaName><CRLNameSuffix><DeltaCRLAllowed>.crl
    - In CDP Erweiterung des ausgestellten Zertifikats einbeziehen
#>
...
<# AIA (Zertifikatsinformationen) konfigurieren
    ...
    ldap:  
    - Eintrag entfernen, und an die Domäne angepassten Eintrag setzen (z.B. spielwiese.local)
    - ldap:///CN=<CATruncatedName>,CN=AIA,CN=Public Key Services,CN=Services,CN=Configuration,DC=MOPW-CENTER,DC=DE
    - In AIA Erweiterung des ausgestellten Zertifikats einbeziehen

    http:// 
    - Eintrag entfernen, und an die Domäne angepassten Eintrag setzen (DNS Alias „pki“ auf Webserver)
    - http://pki.mopw-center.de/CertEnroll/<ServerDNSName>_<CaName><CertificateName>.crt
    - In AIA Erweiterung des ausgestellten Zertifikats einbeziehen
...
Install-AdcsCertificationAuthority -CAType EnterpriseSubordinateCA `
                                   -HashAlgorithmName SHA256 `
                                   -CryptoProviderName 'RSA#Microsoft Software Key Storage Provider' `
                                   -CACommonName MOPW-CENTER-SUBCA `
                                   -ParentCA MOPW-VROOTCA\MOPW-CENTER-ROOTCA -OverwriteExistingKey
...
<# CDP (Sperrliste) konfigurieren
    ...
    file:\\
    - file:\\pki.mopw-center.de\Certenroll\<CaName><CRLNameSuffix><DeltaCRLAllowed>.crl
    - Sperrliste an diesem Ort veröffentlichen
    - Deltasperrlisten an diesem Ort veröffentlichen
#>
...
<# AIA (Zertifikatsinformationen) konfigurieren
    ...
    ldap:  
    - Eintrag entfernen, und an die Domäne angepassten Eintrag setzen (z.B. mopw-center.de)
