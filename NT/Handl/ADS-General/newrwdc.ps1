<#
	...
	.EXAMPLE
		. .\newrwdc.ps1
		New-ADForest -Computername TestDC -ParentDiskPath c:\PD\2008R2\2008R2.vhdx -VMPath v:\VMs -IPv4Address 192.168.1.1 -SubnetMaskLenght 24 -ForestName TestForest -ForestMode Win2008R2
	...
	.NOTES
		...
		Version 1.0.6 - 01/29/2018 - Martin Handl - Subnet mask bits implemented
...
function New-ADForest
{
	...
	begin
	{
		...
		Write-Verbose "Subnetmask is $SubnetMaskLength"
		...
		$decimalSN = ConvertTo-Mask -Mask $SubnetMaskLength
		...
		switch ($LegacyDC)
		{
			true {
				...
				Add-Content -Path "$VMPath\$Computername\Virtual Hard Disks\$Computername.cmd" -Value "netsh interface ipv4 set address `"Local Area Connection`" static $IPv4Address $decimalSN $GW 1"
				...
				Add-Content -Path "$VMPath\$Computername\Virtual Hard Disks\ADCare.ps1" -Value "Rename-ADObject -NewName $SiteName -Identity `"CN=Default-First-Site-Name,CN=Sites,CN=Configuration,`$((Get-ADDomain).DistinguishedName)`""
				Add-Content -Path "$VMPath\$Computername\Virtual Hard Disks\ADCare.ps1" -Value "New-ADObject -Path `"CN=Subnets,CN=Sites,CN=Configuration,`$((Get-ADDomain).DistinguishedName)`" -Name `"$threeocts.0/$($SubnetMaskLength)`" -Type subnet -OtherAttributes @{ siteObject = `"CN=HQ,CN=Sites,CN=Configuration,`$((Get-ADDomain).DistinguishedName)`"  }"
			...
			default
			{
				...
				Add-Content -Path "$VMPath\$Computername\Virtual Hard Disks\ADCare.ps1" -Value "Rename-ADObject -NewName $SiteName -Identity `"CN=Default-First-Site-Name,CN=Sites,CN=Configuration,`$((Get-ADDomain).DistinguishedName)`""
				Add-Content -Path "$VMPath\$Computername\Virtual Hard Disks\ADCare.ps1" -Value "New-ADReplicationSubnet -Name `"$threeocts`.0/$($SubnetMaskLength)`" -Site $SiteName"
				Add-Content -Path "$VMPath\$Computername\Virtual Hard Disks\ADCare.ps1" -Value "Remove-ItemProperty -Path `'HKLM:\Software\Microsoft\Windows NT\CurrentVersion\Winlogon`' -Name AutoAdminLogon"
				Add-Content -Path "$VMPath\$Computername\Virtual Hard Disks\ADCare.ps1" -Value "Remove-ItemProperty -Path `'HKLM:\Software\Microsoft\Windows NT\CurrentVersion\Winlogon`' -Name DefaultUserName"
				Add-Content -Path "$VMPath\$Computername\Virtual Hard Disks\ADCare.ps1" -Value "Remove-ItemProperty -Path `'HKLM:\Software\Microsoft\Windows NT\CurrentVersion\Winlogon`' -Name DefaultPassword"
				Add-Content -Path "$VMPath\$Computername\Virtual Hard Disks\ADCare.ps1" -Value "Remove-ItemProperty -Path `'HKLM:\Software\Microsoft\Windows NT\CurrentVersion\Winlogon`' -Name DefaultDomain"
