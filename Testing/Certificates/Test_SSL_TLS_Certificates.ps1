...
foreach ($url in $urls)
{
    ...
    {
        Write-Host "$certExpiresIn Tage verbleibend für das Zertifikat von $url`nthumbprint: $certThumbprint"
    }
    else
    {
        Write-Host "$certExpiresIn Tage verbleibend für das Zertifikat von $url. Details:`n`nZertifikat Name: $certName`npublic key: $certPublicKeyString`nserial number: $certSerialNumber`nthumbprint: $certThumbprint`neffective date: $certEffectiveDate`nissuer: $certIssuer"
        $errorcounter++
    }
    Remove-Variable req
