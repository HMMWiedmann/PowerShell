...
try
 {
  ...
  Write-ACMELog "System" "Info" "Certificate Assistant Exchange 2010 Version"
...
try {
 ...
 Write-ACMELog "IIS" "Info" "Successfully created .Well-Known Directory"
  ...
  $ErrorDetails = $_.Exception.Message
  Write-ACMELog "IIS" "Info" "Ereor Creating and enabling Well-Known Folder: $ErrorDetails"
}
 ...
 try
  {
   ...
   $AutodiscoverFQDN = ((Get-ClientAccessServer -Identity $ExchangeServer).AutoDiscoverServiceInternalUri.Host).ToLower()
...
try
 {
  ...
  Import-ExchangeCertificate -FileData ([Byte[]]$(Get-Content -Path $CertPath -Encoding byte -ReadCount 0)) -FriendlyName $SANAlias -Password $ImportPassword -PrivateKeyExportable:$true | Enable-ExchangeCertificate -Services "SMTP, IMAP, POP, IIS" �force
