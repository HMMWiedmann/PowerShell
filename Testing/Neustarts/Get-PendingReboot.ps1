...
[object[]]$Pendings = Get-PendingReboot

if($Pendings.WindowsUpdate)
{
    if(($Pendings.WindowsUpdate).GetType().Name -eq "Boolean")
    {
        [string]$WindowsUpdate = $Pendings.WindowsUpdate
    }
}
...
[int32]$rebootdays = ((get-date) - $os.ConvertToDateTime($os.LastBootUpTime)).Days
if($paraDays -ne $null)
{
    if($rebootdays -gt $paraDays)
    {
        $errorcounter++
    }
}

# Write-Host "Errorcount : $errorcounter" -ForegroundColor Red