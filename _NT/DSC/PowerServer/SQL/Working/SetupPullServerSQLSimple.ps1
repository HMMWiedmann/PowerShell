function Setup-DSCWebPullServer
{
    ...
    begin
    {
        ...
        if($SQLProvider -eq "True")
        {
            ...
            {
                $SQLConnectionString = "Provider=SQLNCLI11;Integrated Security=SSPI;Persist Security Info=False;Initial Catalog=master;Data Source=$env:COMPUTERNAME\$SQLInstanceName;Database=$SQLDBName"
            }              
            #endregion
    ...
    process
    {
            ...
            Configuration SetupDSCPullServerWeb
            {
                ...
                Node $env:COMPUTERNAME
                {
                    ...
                    xDscWebService PSDSCPullServer 
                    {
                        ...
                        ModulePath               = "$DSCServicePath\Module"
        ...
        Start-DscConfiguration -Path $PullSrvConfig -Verbose -Wait -Force
    }
    
    end
    {
        $ServerURL = "https://" + $env:COMPUTERNAME + '.' + $env:USERDNSDOMAIN + ':' + $FirewallPort + '/' + $EndPointName + '.svc'

        $WebsiteResult = Invoke-WebRequest -UseBasicParsing -Uri $ServerURL
